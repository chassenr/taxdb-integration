rule download_proteins_ncbi:
	...
	shell:
		...
		awk -v FS="\\t" -v OFS="\\t" '$3 == "TRUE"' {input.url} | cut -f2 | grep -F -f <(cut -f1 {input.tax}) > "{params.outdir}/links.list"
		...
		then
		  cut -d'_' -f1,2 "{params.outdir}/tmp1" | grep -F -f - {input.tax} > {output.tax_prot}
		fi
